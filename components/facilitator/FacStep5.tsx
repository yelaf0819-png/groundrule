...
export default function FacStep5({ session }: { session: Session }) {
  ...
  const [rules, setRules] = useState<FinalRule[]>(() => {
    ...
    return counts.slice(0, 4).map((c, i) => ({ id: c.id, text: c.text, order: i + 1 }));
  });
    ...
    <div className="space-y-4">
      <div className="bg-white border border-stone-200 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          {!confirmed && (
            <p className="text-xs text-stone-500">클릭해서 문구를 다듬어주세요</p>
          )}
        </div>
                ...
                <div className="flex items-start gap-3 px-4 py-3">
                  ...
                  {!confirmed && (
                    <div className="flex gap-1 flex-shrink-0">
                      <button onClick={() => startEdit(rule)} className="p-1.5 text-stone-400 hover:text-stone-700">
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => deleteRule(rule.id)} className="p-1.5 text-stone-400 hover:text-rose-600">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  )}
        ...
        {!confirmed ? (
          ...
          <div className="mt-5 bg-emerald-50 rounded-xl p-4 text-center">
            ...
            <p className="font-bold text-emerald-900">우리 팀의 그라운드룰이 확정됐습니다!</p>
